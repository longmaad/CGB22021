...
//			for(int i=0;i<=files.length();i++) {//1
			for(int i=0;i<files.length;i++) {
				//获取每一个子项的名字
				//String的类名拼写错误
//				string fileName = files[i].getName();//String类名拼写错误
				String fileName = files[i].getName();
				//判断名字中是否包含key的内容
				//1:filename变量名拼写错误
//				if(filename.不知道什么方法) {//fileName变量名拼写错误
				if(fileName.contains(key)) {
					//包含就输出这个名字
//					System.out.println(f1leNeme);//输出fileName拼写错误
